...
class seed : public BASE {
    ...
    /// Quality of the seed (only meaningfull in comparson) (non-const)
    ///
    /// @return A (const) vector of <tt>float</tt> values
    ///
    TRACCC_HOST_DEVICE
    auto& qual() { return BASE::template get<3>(); }
    /// Quality of the seed (only meaningfull in comparson) (const)
    ///
    /// @return A (const) vector of <tt>float</tt> values
    ///
    TRACCC_HOST_DEVICE
    const auto& qual() const { return BASE::template get<3>(); }
