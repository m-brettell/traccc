    ...
    if (edge_idx < nEdges) {
        ...
        if (out_paths.y != -1) {
            d_outgoing_paths[edge_idx].y = atomicAdd(&d_counters[7], 1);
						if(out_paths.x < 0) printf(" wi %i ", out_paths.x);
            atomicAdd(&outgoingCount, out_paths.x);
        }
    }
    __syncthreads();
    if (threadIdx.x == 0) {
				if(outgoingCount < 0) printf(" hi %i ", outgoingCount);
    ...
    if (threadIdx.x < 16 && path_idx < nTerminus) {
        ...
        for (int nei = 0; nei < nNei; ++nei) {
            ...
            int new_path_idx = atomicAdd(&d_counters[7], 1);
						if(path_idx == d_counters[6]+nTerminus) {printf(" maxhit 2 "); continue;}
...
void __global__ fit_segments(
    ...
    unsigned int nTerminusEdges, unsigned int nPaths, int minLevel,
		unsigned int max_num_neighbours,
